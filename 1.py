...
try:
    ...
    print("⚠️ ꜱᴄɪᴘʏ ɴᴏᴛ ɪɴꜱᴛᴀʟʟᴇᴅ - ʙᴀꜱɪᴄ ᴀᴜᴅɪᴏ ᴘʀᴏᴄᴇꜱꜱɪɴɢ ᴏɴʟʏ")
...
def apply_treble_boost_basic(audio_data, treble_level):
    ...
    # Smooth boost
    mix = min(0.50, treble_level / 120.0)
...
def apply_soft_gain(audio_data, gain_level):
    ...
    # ==================== Input Gain ====================
    gain_factor = 4.0 + (gain_level / 20.0)
...
"""
        ...
        buttons = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("🔄 ʀᴇꜰʀᴇꜱʜ", callback_data="refresh_userlist"),
                InlineKeyboardButton("📊 ꜱᴛᴀᴛꜱ", callback_data="userlist_stats"),
            ],
            [
                InlineKeyboardButton("⬅️ ʙᴀᴄᴋ", callback_data="back_start"),
            ]
        ])
...
class ButtonStyle:
    """
    Telegram's Bot API has no colored/styled inline-button feature — these
    constants are kept only as readability labels in the button-layout lists
    below (e.g. marking a button as "the dangerous one"); they have no visual
    effect and are not sent to Telegram.
    """
    PRIMARY = "bg_primary"
    SUCCESS = "bg_success"
    DANGER = "bg_danger"
...
def styled_button(text: str, callback_data: str = None, url: str = None, style: str = ButtonStyle.PRIMARY) -> InlineKeyboardButton:
    """ᴄʀᴇᴀᴛᴇ ᴀ ʙᴜᴛᴛᴏɴ (ꜱᴛʏʟᴇ ᴀʀɢ ɪꜱ ᴄᴏꜱᴍᴇᴛɪᴄ/ᴅᴏᴄᴜᴍᴇɴᴛᴀᴛɪᴏɴ ᴏɴʟʏ, ꜱᴇᴇ ButtonStyle)"""
    if url:
